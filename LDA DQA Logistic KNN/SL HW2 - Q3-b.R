# Q3 -b : NLP for Spam or Ham with 500 SMS using last 4 digits of student ID
...
# Pick 500 SMS based on last 4 digit of my student ID
set.seed(6145)
pick <- sample(1:nrow(SMSS_DF), 500)

# Parse data randomly into 80% Training and 20% Testing
TrainRandom <- sample(1:length(pick), as.integer(length(pick)*0.8), replace=F) 
TestRandom <- setdiff(pick, TrainRandom)     
...
SpamFrqGraph <- ggplot(Spam_term_DF[1:10,], aes(x=reorder(Spam_term_DF$word[1:10], -(Spam_term_DF$freq[1:10])), Spam_term_DF$freq[1:10]), color=Spam_term_DF$freq[1:10]) + geom_bar(stat="identity", aes(fill = Spam_term_DF$freq[1:10])) + geom_text(aes(label = Spam_term_DF$word[1:10]), hjust = 1.05, srt=90, size = 4, color = "white", fontface= "bold") + theme(axis.text.x=element_blank())
...
HamFrqGraph <- ggplot(Ham_term_DF[1:10,], aes(x=reorder(Ham_term_DF$word[1:10], -(Ham_term_DF$freq[1:10])), Ham_term_DF$freq[1:10]), color=Ham_term_DF$freq[1:10]) + geom_bar(stat="identity", aes(fill = Ham_term_DF$freq[1:10])) + geom_text(aes(label = Ham_term_DF$word[1:10]), hjust = 1.05, srt=90, size = 4, color = "white", fontface= "bold") + theme(axis.text.x=element_blank())
...
word_DTM_Train <- DocumentTermMatrix(corpus, list(global = c(2, Inf), dictionary = all_features))
