...
KCWeather = read.csv("kc_weather_data.csv", header=TRUE)
head(KCWeather)
...
SVM_Model <- train(Events ~ Temp.F + Dew_Point.F + Humidity.percentage + Sea_Level_Press.in + Visibility.mi + Wind.mph + Precip.in + Year + Month + Day, data = TrainSet, method = 'svmLinear3', repeats=100)

# Test SVM Model
SVM_Predict <- predict(SVM_Model, TestSet)

# Fine tune SVM - optimized cost and gamma
SVM_Tune <- tune(svm, Events ~ Temp.F + Dew_Point.F + Humidity.percentage + Sea_Level_Press.in + Visibility.mi + Wind.mph + Precip.in + Year + Month + Day, data = TrainSet, ranges = list(epsilon = seq(0,0.2,0.01), cost = 2^(2:9)))

SVM_Model_T <- train(Events ~ Temp.F + Dew_Point.F + Humidity.percentage + Sea_Level_Press.in + Visibility.mi + Wind.mph + Precip.in + Year + Month + Day, data = TrainSet, method = 'svmLinear3', cost = 4, gamma= 0, repeats=100)
